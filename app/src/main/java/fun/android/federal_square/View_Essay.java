...
import fun.android.federal_square.fun.Fun_删除文章;
...
public class View_Essay extends AppCompatActivity {
    ...
    public void 初始化数据(){
        ...
        for(int i=0;i<list.size();i++){
            try {
                ...
                linear.post(()->{
                    linear.addView(创建文章贴子(post_data));
                });
            }catch (Exception e){
                Fun_文件.删除文件(able.app_path + "Square_Data/" + list.get(i));
            }
    ...
    private View 创建文章贴子(List<Post_Data> post_data){
        String time_name="";
        ...
        button_collection.setOnClickListener(V->{
            NetWork_添加收藏 netWork_添加_收藏 = new NetWork_添加收藏(this);
            netWork_添加_收藏.传递参数(finalTime_name1, post_data);
            netWork_添加_收藏.start();
        });
        String finalTime_name2 = time_name;
        view.setOnLongClickListener(V->{
            Fun_删除文章.所有启动(this, finalTime_name2);
            return true;
        });
