...
public class 选择头像窗口 {
    ...
    public static void 选择头像(Activity activity, ImageView avatar_img){
        ...
        disk_grid_adapter = new Disk_Grid_Adapter(activity, file_list);
        ...
        gridview.setOnItemClickListener((adapterView, view1, position, l) -> {
            ...
            if(!post_dataList.isEmpty()){
                ...
                netWork_头像_上传.传递参数(post_dataList, able.URL + "federal-square/Account/" + Fun_账号.GetID() + "/Image_Resources/" + file_list.get(position), avatar_img, dialog);
                netWork_头像_上传.start();
            }
        });
